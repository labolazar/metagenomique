...
Some program may take some time to process, we therefor recommend running the command using a shell script with nohup. Exemple : 
```{bash, eval=FALSE}
vi mapping.sh # This creates a blank document in a vi text editor that is called mapping.sh to start editing the document press i 
```
...
1. Create a vi document called /mapping.sh & 
